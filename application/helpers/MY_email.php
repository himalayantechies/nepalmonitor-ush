...
class email extends email_Core {
	...
	public static function send($to, $from, $subject, $content, $html = FALSE)
	{
		...
		$header_type = ($config['sendAs'] == 'text') ? 'text/plain' : 'text/html';
		$html = ($config['sendAs'] != 'text') ? TRUE : FALSE;
		$plain = ($config['sendAs'] != 'html') ? TRUE : FALSE;

		// Create the message
		$message = new Swift_Message($subject, '', $header_type, '8bit', 'utf-8');
		if ($plain === TRUE)
		{
		...
		if ($html === TRUE)
		{
			$message->attach(new Swift_Message_Part(html::clean(nl2br($content)), 'text/html'));
		}
		if (is_string($to))
		{
			// Single recipient
