...
class S_Alerts_Controller extends Controller {
	...
	public function index()
	{
		...
		$switch_message = Kohana::lang('alerts.switch_alert')
								.url::site().'alerts/alert_switch/';
...
/*		$incidents = $db->query("SELECT i.id, incident_title,
       ...
       $incidents = $db->query($incident_query);
		...
		foreach ($incidents as $incident)
		{
			...
			$incident_html_description = $html2text->get_text(); // HT: New Code to make text only for sms alert
			...
			$sms_message = $incident_html_description; // HT: New Code to use text for sms
			// Remove line breaks
			...
			// Get all alertees
			//$alertees = $db1->query("SELECT * FROM ".$this->table_prefix."alert AS a WHERE a.alert_confirmed = 1 AND a.id NOT IN(SELECT as.alert_id FROM ".$this->table_prefix."alert_sent AS as WHERE as.incident_id = ".$incident->id.")");
			// HT: New Code
			$alert_sent = ORM::factory('alert_sent')->where('incident_id', $incident->id)->select_list('id', 'alert_id');
			// $alertObj = ORM::factory('alert')->where('alert_confirmed','1')->where('id', '241'); // HT: remove where after testing
			$alertObj = ORM::factory('alert')->where('alert_confirmed','1');
			if(!empty($alert_sent)) $alertObj->notin('id', $alert_sent);
               ...
               if($this->_multi_subscribe($alertee, $incident->id)) {
				...
				if ($distance <= $alert_radius)
				{
					...
					elseif ($alert_type == 2 && ($incident->alert_mode == 0 || $incident->alert_mode == 1)) // Email alertee
					{
						...
						.$alertee->alert_code
						.'<br/>'
						.$switch_message.$alertee->alert_code
						.'<br/>'
