	...
	foreach ($disp_custom_fields as $field_id => $field_property)
	{
		...
		if ($field_property['field_type'] == 1)
		{
			...
			if (isset($field_options['field_hidden']) AND !isset($editor))
			{
				...
				else
				{
					...
					if($data_type == 'numeric') {
						echo form::input(array('name' => 'custom_field['.$field_id.']', 'type' => 'text', 'id' => 'custom_field_'.$field_id, 'value' => $field_value, 'class' => 'text custom_text', 'pattern' => '(NA)|(na)|(N/A)|(n/a)|([Uu][Nn][Kk][Nn][Oo][Ww][Nn])|[0-9]{0,}'));
					} else {
			...
			else
			{
				echo "<h4>" . $field_property['field_name'] . $isrequired . " " . $isprivate . "</h4>";
				if($data_type == 'numeric') {
					echo form::input(array('name' => 'custom_field['.$field_id.']', 'type' => 'number', 'id' => $id_name, 'value' => $field_value, 'class' => 'text custom_text', 'pattern' => '(NA)|(na)|(N/A)|(n/a)|([Uu][Nn][Kk][Nn][Oo][Ww][Nn])|[0-9]{0,}'));
				} else {
