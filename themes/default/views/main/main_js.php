 ...
 */
//function refreshTimeline() { // HT: updated on new ushahidi code
function refreshTimeline(options) {

	<?php if (Kohana::config('settings.enable_timeline')) {?>

	// var options = (arguments.length == 0) ? {} : arguments[0]; // HT: commented on new ushahidi code
	// HT: new ushahidi code
	...
	url += (options.c !== undefined && parseInt(options.c) > 0) ?  options.c : ''; // HT: commented on new ushahidi code
...
jQuery(function() {
	...
	//setTimeout(function() { refreshTimeline(); }, 1500); // HT: commented due to new code from usha
	// HT: new code from ushahidi
